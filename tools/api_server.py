...
import auth as _auth  # per-user identity (bookmarks, activity, soft-delete). Opt-in: layered on Basic Auth.
...
# ---------------------------------------------------------------------------
# Per-user actions: bookmarks + soft-delete lead
# All require a logged-in session. Restored 2026-08-31 from feat/auth 508a1aad.
# ---------------------------------------------------------------------------
...
# Restored 2026-08-31 from feat/auth 508a1aad. Uses /api/auth/session-login
# to coexist with the existing /api/auth/login tracker below.
# ---------------------------------------------------------------------------
...
async def session_login(body: _LoginBody, request: Request) -> JSONResponse:
    """Per-user session login. The allowlist is the TEAM_USERS env var
    (comma-separated). On success, sets the bsz_sid HttpOnly cookie and
    returns the user record. Distinct from /api/auth/login (the AccessGate
    login-event tracker)."""
