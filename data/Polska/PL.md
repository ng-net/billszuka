# 🇵🇱 Polska — Dziennik Badawczy & Status

> **Status:** ✅ ZWERYFIKOWANE & ZSYNCHRONIZOWANE (Dane w `master.csv`)  
> **Katalog:** 237 firm PL (`catalog-A-PL.csv` 31 firm + `catalog-B-PL.csv` 206 firm)  
> **Kluczowe podmioty 🐋:** BILLS Sp. z o.o., BISTA Standard, Polski Tytoń SA, Polska Grupa Tytoniowa, CK Complex, ZOLTA Trade, PRIMA-TECH, P&P Cigarro, JAS-FBG S.A., ROHLIG SUUS Logistics S.A.  



## Reżim regulacyjny (stan: 2024-2025)
- Akcyza na tytoń — wysoka, rosnąca
- Zakaz reklamy wyrobów tytoniowych
- Ograniczenia sprzedaży: 18+, automaty zakazane
- E-papierosy: legalne, dyskusja o ograniczeniach smakowych (obserwować 2025-2026)
- CBD: w szarej strefi regulacyjnej, susz z limitem THC
- Nabijarki: brak ograniczeń jako urządzenia

## Rejestry i bazy
- **NIP** (10 cyfr)
- **CEIDG** — publiczna wyszukiwarka https://www.ceidg.gov.pl (bez tokena)
- **KRS** — https://www.krs-online.com.pl (płatny), https://ekrs.ms.gov.pl (oficjalny)
- **GUS / REGON** — baza statystyczna
- **VIES** — walidacja VAT EU
- **PKD kody do przeszukania**: 46.35Z (hurt tytoniowy), 46.17Z (pośrednictwo), 47.11Z (sklepy), 47.29Z (pozostała sprzedaż detaliczna żywność), 47.19Z (pozostałe sklepy), 47.91Z (sprzedaż wysyłkowa)

## Marketplaces
- **Allegro** (gigant — must-have)
- **OLX** (ogłoszenia, nie tylko sprzedaż)
- **Ceneo** (porównywarki cen)
- **Kaufland** (marketplace, rośnie)
- **InPost Buy** (nowy, rośnie szybko)
- **Erli** (mniejszy)

## Firmy zidentyfikowane (wstępnie)

### Katalog A — z nabijarkami

| Firma | Kategoria | Tier | Uwagi |
|---|---|---|---|
| **BILLS Sp. z o.o.** (Ostrzeszów) | A1 | Exclusive | ✅ BILLS-LIKE — własna firma. Kontakt: hurt@bills.pl, sales@bills.pl |
| **Tobacchem** (woj. lubelskie?) | A4 | Reseller | Hurtownia, dystrybutor akcesoriów. tobacchem.pl — ma sklep + B2B |
| **Don Marco** | A4 | Reseller | Lider PL B2B, importer, roszcze do "lider w Europie" — warto zweryfikować |
| **Bista Standard** (Bydgoszcz) | A5/A4 | Manufacturer | Właściciel Dark Horse + FERN, eksport do 70 krajów. **🐋 BIG FISH** — sprawdzić czy mają własne maszynki |
| **Cut and Roll S.C.** (Polska) | A5 | Manufacturer | Własne maszynki do cięcia tytoniu, hurt + export |
| **Mega Import** | A4 | Wholesaler | megaimport.pl, hurtownia akcesoriów |
| **Powermatic Store** (Erli, Allegro) | A1 | Marketplace | Sklep dedykowany — prawdopodobnie pod skrzydłami BILLS lub autoryzowany |
| **Konopny_sklep** (Allegro) | A1? | Marketplace | Sprzedaje POWERMATIC III+, deklaruje dystrybucję BILLS |
| **Dopalenia.pl** | A4 | Retailer | domena "dopalenia.pl", sklep + hurtownia |
| **Shishka79.pl** | B5/B4 | Retailer | Sklep shisha + akcesoria — kandydat cross-sell |
| **Topomat**, **Turbomatic**, **GM** (firmy) | A5 | Marketplace | Prawdopodobne firmy-importorzy OEM z Chin |

### Katalog B — bez nabijarek (wstępnie)

| Firma | Kategoria | Powinowactwo | Uwagi |
|---|---|---|---|
| **Luna Corporate** (Suchy Las) | B7 | 2 | Właściciel 77, Bjorn, Maverick — nicotine pouches |
| **FREGA FREJOWSK, GARABOL** (Kielce) | B8 | 5 | Hurtownia tytoniowa |
| **Hurtownia Papierosów LORD** (Słubice) | B8 | 5 | 5-9 pracowników, hurtownia |
| **RUCH SA Hurtownia** (Kielce) | B8 | 5 | Duży gracz, 200-499 pracowników |
| **Grey Art Development** (Łódź) | B8 | 5 | Hurtownia |
| **Carmen Polska** (Sochaczew) | B8 | 5 | Hurtownia, 100-199 pracowników |
| **SEMI** (Radom) | B8 | 5 | Hurtownia |
| **Anlex Sp. z o.o.** (Białobrzegi) | B8 | 5 | Hurtownia z dostawą worldwide |
| **Polski Tytoń S.A.** (Radom) | B8 | 5 | **🐋 BIG FISH** — 1947, 1000+ pracowników |

## Niespodzianki / ciekawostki
- **Bista Standard** właściciel **Dark Horse** i **FERN** — własne marki akcesoriów. Eksport do 70 krajów. Silny kandydat na partnera lub benchmark konkurencyjny
- **Don Marco** deklaruje "lider w Europie" — warto zweryfikować, czy to nie jest przesadzone
- BILLS jest **exclusive** na PM dla PL + CEE — to rzadka pozycja, warto ją dobrze wykorzystać

## Czerwone flagi
- **Don Marco** — twierdzenia o pozycji lidera w Europie, brak niezależnego potwierdzenia
- **Powermatic Store** / **Konopny_sklep** — sprzedają PM, ale nie wiadomo czy mają umowę z BILLS czy szarą strefą
- "Turbomatic", "GM", "Topomat" — marki-producenta nie zidentyfikowałem, prawdopodobnie import OEM

## Gemy (off-internet) — do zbadania
- Grupy FB: "Nabijarki do tytoniu", "PowerMatic Polska", "Tytoń do skręcania"
- YouTube: recenzenci maszynek (wyszukać i prześledzić łańcuch dostaw)
- OLX: ogłoszenia z całej Polski (szukać sprzedawców z powtarzającymi się ogłoszeniami)

## Źródła do dalszego research
- Europages: https://www.europages.de/unternehmen/polen/großhändler/tabak.html (11 firm)
- Facebook grupy: https://www.facebook.com/groups/ (szukać po frazach)
- Allegro: wyszukać "powermatic" → lista sprzedawców
- Google Maps: "sklep tytoniowy" w dużych miastach
- Bista Standard: https://www.intertabac.de/en/as-profil/bista-standard-sp-z-o-o-2/

## Synteza 2026-08-10 — quality data + akcje

### Stan katalogu BILLSzuka PL (zweryfikowany rdzeń)

| Katalog | Rows | FROZEN | DOV | PENDING | QS avg | Email | Phone | WWW | Decydent |
|---|---|---|---|---|---|---|---|---|---|
| catalog-A-PL | 3 | 3/3 (100%) | 0 | 0 | 86.7 | 67% | 67% | 100% | 33% |
| catalog-B-PL | 27 | 25/27 (93%) | 2 | 0 | 79.1 | 41% | 41% | 41% | 22% |

**Werdykt:** Rdzeń zweryfikowany (28/30 FROZEN). 2 DO-WERYFIKACJI to real
mismatches (jaccard <0.8 z ARES) — nie false-positives, tylko firmy których
nazwa w CSV nie pasuje do nazwy w KRS/CEIDG. **Baza gotowa do outreachu
dla 28 firm**, ale wymaga wzbogacenia o brakujące email/phone/www/decydent.

### Szerszy bas leadów (do enrichementu)

| Źródło | Rows | A1/A2 hot | B1/C nurturing | D excluded | NIP | KRS | Decydent |
|---|---|---|---|---|---|---|---|
| `BILLS-SMOKS-Research-2026/07-MASTER-Katalog-...` | 376 | 24 + 39 = **63** | 235 | 78 | 20% | 6% | 4% |
| `Clients/Bills/Research/Polska/07-MASTER-...` | 218 | 40 + 60 = **100** | 118 | 0 | 16% | 7% | 0% |

**Werdykt:** Pula 594 leadów; **163 hot** (A1+A2) z dwóch master katalogów.
Ale prawie wszystkie wymagają enrichementu (NIP/KRS/decydent). To jest
następna fala pracy — najpierw enrich A1/A2 (~163 firm), potem B1.

### Akcje (instrukcje dla producenta / następna sesja)

#### 🔴 P0 — outreach-ready (ten tydzień)
- **A1. Outreach 28 FROZEN PL firm** z `catalog-A/B-PL.csv`. Wymagane
  kolumny: nazwa, email (gdy ma), telefon (gdy ma), www, adres, decydent
  (gdy ma). Owner: Marceli.
- **A2. Investigate 2 DO-WERYFIKACJI** (jaccard mismatch na ARES) — to mogą
  być albo nazwy z literówkami albo prawdziwe FABRYKAT. Sprawdzić ręcznie
  KRS po nip_vat, poprawić `nazwa` albo usunąć wiersz.

#### 🟡 P1 — enrichement (1-2 tygodnie)
- **A3. Back-fill 16 BILLSzuka PL rows missing email/phone/www** —
  uruchomić `tools/verify_lead.py` po tych wierszach (whois domeny →
  email pattern; LinkedIn scrape → decydent). Wzorzec:
  `python3 tools/verify_lead.py --country PL --rows catalog-B-PL.csv`.
- **A4. Enrich 63 hot leads (A1+A2) z BILLS-SMOKS-Research** — przekonwertować
  format tego mastera do BILLSzuka schema, uruchomić KRS/CEIDG lookup
  po NIP (lub NIP discovery po nazwie przez CEIDG `nazwa`), potem
  verify_api --country PL --all. Target: 30-50% powinno dojść do FROZEN
  po enrichemencie.
- **A5. Enrich 100 hot leads (A1+A2) z Research/Polska** — j.w. To są
  hard-targets, wyższy conversion rate niż A4 (research był bardziej
  focused). Sprawdzić czy nie ma overlap z BILLS-SMOKS-Research przed
  dedupem.

#### 🟢 P2 — schema i tooling (audyt 18:41)
- **A6. (NOT for me — for producer)** Atomiczne CSV writes w 6 skryptach
  (verify_api.py ×3, verify_run.py ×1, ~~l0_preflight.py ×1~~ — usunięty
  2026-09-03, fix_data_quality.py ×1, extract_intel.py ×1). Wzorzec: tmp → os.replace
  (jak `regenerate_master()` w verify_run.py:387-397). Kill mid-write
  obecnie może uciąć CSV i stracić dane.
- **A7. ~~(NOT for me — for producer)~~ DONE 2026-09-03** `tools/verify_lead.py`
  strukturalnie niekompletny — 2/3 tools zwracają zawsze PENDING.
  Rozwiązane: skrypt usunięty, logika dwu-narzędziowa przeniesiona do
  agenta (web_search + whois on demand).
  Albo zaimplementować prawdziwy web_search backend albo zdowngradować
  log line żeby nie wprowadzał w błąd. Checkpoint write też powinien
  być atomiczny.
- **A8. ~~(NOT for me — for producer)~~ DONE 2026-09-03** `l0_preflight.py:289`
  hardcoded `"2026-08-10"`. Rozwiązane: skrypt usunięty (Session 2 planu
  verify-consolidation), logika przeniesiona do `verify_api.py --retrofix`.
- **A9. (NOT for me — for producer)** Hardcoded
  `/Volumes/MC-BRAIN/Dev-Ext/BILLSzuka` w `l0_preflight.py:37` +
  `run_verify_cron.sh:6` — zamienić na `Path(__file__).resolve().parent.parent`
  (l0) i `$(dirname "$0")/..` (cron). Inaczej nie przeniesiesz projektu
  na inny mount bez edycji. (Częściowo rozwiązane: `l0_preflight.py`
  usunięty, ale `run_verify_cron.sh` wciąż ma hardcoded path.)
- **A10. ~~(NOT for me — for producer)~~ DONE 2026-09-03** Dead code
  w `l0_preflight.py` (linie 267-268, 274-276, 329-333). Rozwiązane:
  skrypt usunięty, cały martwy kod zniknął razem z nim.

#### ⚪ P3 — nice to have
- **A11. Cross-reference** BILLSzuka PL catalog z dwoma master katalogami —
  dodać kolumnę `źródło` (BILLSzuka / BILLS-SMOKS-Research / Research/Polska)
  żeby wiedzieć skąd każdy lead pochodzi.
- **A12. Outreach 78 D-priority leads** (BILLS-SMOKS-Research) —
  prawdopodobnie skip (stąd D), ale warto przejrzeć czy nie ma tam
  hidden gems.
- **A13. PL.md update po enrichemencie** — dodać sekcję "Firmy po
  enrichemencie 2026-XX-XX" z nowymi FROZEN.

### Statystyki na dziś (2026-08-10 18:55)

- **BILLSzuka PL: 28/30 FROZEN** (93%), QS avg 79-87
- **Master catalogs: 594 unverified** (163 hot, 235 nurture, 78 skip)
- **Tests: 92 PASS** (verify + l0)
- **Audit verdict: 6 issues, 2 P0 (data loss), 4 P2 (smell)** — see A6-A10

### Cross-country context (dla porównania)

| CC | Katalog | FROZEN | DOV | PENDING | Status |
|---|---|---|---|---|---|
| 🇵🇱 PL | 30 | 28 | 2 | 0 | Ready for outreach |
| 🇨🇿 CZ | 7 | 6 | 1 | 0 | Almost done |
| 🇪🇪 EE | 10 | 8 | 2 | 0 | Almost done |
| 🇱🇹 LT | 10 | 1 | 0 | 9 | PENDING_API (no ja_kodas) |
| 🇫🇷 FR | 11 | 0 | 0 | 11 | Needs re-verify with FR rich API |
